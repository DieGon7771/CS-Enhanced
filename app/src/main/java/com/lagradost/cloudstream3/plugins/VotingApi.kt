...
object VotingApi {
    ...
    private const val API_DOMAIN = "https://voting-api.diegon7771.workers.dev"
    ...
    private suspend fun readVote(pluginUrl: String): Int {
        ...
        val url = "$API_DOMAIN/?id=$id"
        Log.d(LOGKEY, "Requesting GET: $url")
        return app.get(url).parsedSafe<WorkerResult>()?.count ?: 0
    }
    ...
    private suspend fun writeVote(pluginUrl: String): Boolean {
        ...
        return app.post(url, emptyMap<String, String>())
            .parsedSafe<WorkerResult>()?.count != null
    }
